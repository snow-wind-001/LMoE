...
class OpenLLAMAPEFTModel(nn.Module):
    ...
    def __init__(self, **args):
        ...
        self.args = args
        # imagebind_ckpt_path = self.args['imagebind_ckpt_path']
        # vicuna_ckpt_path = self.args['vicuna_ckpt_path']
        # max_tgt_len = self.args['max_tgt_len']
        # stage = args['stage']

        imagebind_ckpt_path = '/home/user/git_code/LMOE/libcode/AnomalyGPT_TEMP/pretrained_ckpt/imagebind_ckpt/imagebind_huge.pth'
        ...
        # self.prompt_learner = PromptLearner(1, 5120)
        ...
        config = AutoConfig.from_pretrained(vicuna_ckpt_path, **config_kwargs)
        self.llama_model = LlamaForCausalLM.from_pretrained(vicuna_ckpt_path,config=config)
    ...
    def forward(self, inputs):
        ...
        if 'masks' in inputs:
            ...
            hidden_state = outputs1.hidden_states
            ...
            response = self.llama_tokenizer.decode(chosen_tokens[0], skip_special_tokens=True)
            labels = targets[:, 2:]
            ...
            return loss + loss_pixel, gen_acc, hidden_state,response
            ...
            return loss, gen_acc, hidden_state,response
    ...
    def generate(self, inputs, web_demo=False):
        ...
        # outputsx = outputs1.hidden_states[-1]
        hidden_state = outputs1.hidden_states
        hidden_state = torch.mean(hidden_state[0].squeeze(0) + hidden_state[-1].squeeze(0), axis=0)
        hidden_state = hidden_state.unsqueeze(0)
        # print(f'outputsx{hidden_state.shape}')
        output_text = self.llama_tokenizer.decode(outputs[0][:-2], skip_special_tokens=True)
        return output_text, pixel_output, hidden_state